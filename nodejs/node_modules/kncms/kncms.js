...
//
  setting: function(op){
    ...
    op.current=op.current||me.CFG.current.substr(0, l-1);
    ...
    try{
      if(!me.getInfoj(base)){
        ...
        if(!me.isExist(base+url)){
          ...
          if(!dt["BODY"]){dt=me.pageinfo(base+"/error404.page", base);}
//          b=dt["PARM"].split("\n");
//         var v;
...
// 
  pankuzu: function(){
    var me=this; var out, u, mem, base, dt=[], i, j, k;
    mem=me.INFOJ['pankuzu_form']||'pankuzu';
    base=me.INFOJ['base'];
    dt[0]={}; dt[0].url='/index.html'; dt[0].title='トップ';
    ...
    if(me.SS.PATH.length==3){
      if(me.SS.PATH[2]!='' && me.SS.PATH[1]!='index.html'){
        ...
        for(j in me.MENU){if(me.MENU[j].url==dt[1].url){dt[1].title=me.MENU[j].short;}}
        dt[2]={}; dt[2].url='/'+me.SS.PATH[1]+'/'+me.SS.PATH[2];
        for(j in me.MENU){if(me.MENU[j].url==dt[2].url){dt[2].title=me.MENU[j].title;}}
      }else{
        dt[1]={}; dt[1].url='/'+me.SS.PATH[1]+'/index.html';
        for(j in me.MENU){if(me.MENU[j].url==dt[1].url){dt[1].title=me.MENU[j].short;}}
        dt[1]={}; dt[2].url='/'+me.SS.PATH[1]+'/index.html';
        for(j in me.MENU){if(me.MENU[j].url==dt[2].url){dt[1].title=me.MENU[j].title;}}
      }
    }
